...
if ((count _buildingNetIdsToSpawnLootIn) > 0) then {
	private _buildingPercent = getNumber (missionConfigFile >> "ExileExtendetLootspawn" >> "buildingGetsLootChance");
	private _buildingCount = floor ((count _buildingNetIdsToSpawnLootIn) * _buildingPercent / 100);
	private _selectedBuildings = _buildingNetIdsToSpawnLootIn select [0,_buildingCount];
